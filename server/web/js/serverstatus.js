	...
	$.getJSON("stats.json", function(result) {
		...
		for (var i = 0, rlen=result.servers.length; i < rlen; i++) {
			...
			if (result.servers[i].status) {
				...
				TableRow.children["status"].children[0].children[0].innerHTML = "<small>Online</small>";
			} else {
				TableRow.children["status"].children[0].children[0].className = "progress-bar progress-bar-danger";
				TableRow.children["status"].children[0].children[0].innerHTML = "<small>Offline</small>";
			}
			...
			if (!result.servers[i].status) {
				...
				// Uptime
				TableRow.children["uptime"].innerHTML = timeformat(result.servers[i].uptime);
